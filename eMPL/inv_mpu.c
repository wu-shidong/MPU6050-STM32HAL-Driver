...
//Gyroscope direction setting
static signed char gyro_orientation[9] = { 1, 0, 0,
                                           0, 1, 0,
                                           0, 0, 1};

/**
  * @brief   MPU6050 Self-testing
  * @param   none
  * @retva   0:fail 1:success
  */
...
/**
  * @brief   Gyroscope direction control
  * @param   none
  * @retva   scalar
  */
...
{
	uint8_t res=0;
	if(mpu_init()==0)	//Init MPU6050
	{	 
		res=mpu_set_sensors(INV_XYZ_GYRO|INV_XYZ_ACCEL);//Set up the required sensors
		if(res)return 1; 
		res=mpu_configure_fifo(INV_XYZ_GYRO | INV_XYZ_ACCEL);//Set FIFO
		if(res)return 2; 
		res=mpu_set_sample_rate(DEFAULT_MPU_HZ);	//Set sampling rate
		if(res)return 3; 
		res=dmp_load_motion_driver_firmware();		//Load the dmp firmware
		if(res)return 4; 
		res=dmp_set_orientation(inv_orientation_matrix_to_scalar(gyro_orientation));//Set the gyroscope direction
		if(res)return 5; 
		res=dmp_enable_feature(DMP_FEATURE_6X_LP_QUAT|DMP_FEATURE_TAP|	//Example Set the dmp function
		    DMP_FEATURE_ANDROID_ORIENT|DMP_FEATURE_SEND_RAW_ACCEL|DMP_FEATURE_SEND_CAL_GYRO|
		    DMP_FEATURE_GYRO_CAL);
		if(res)return 6; 
		res=dmp_set_fifo_rate(DEFAULT_MPU_HZ);	//Setting the DMP output rate (Maximum 200Hz)
		if(res)return 7;   
		res=run_self_test();		//self-inspection
		if(res)return 8;    
		res=mpu_set_dmp_state(1);	//Enable DMP
		if(res)return 9;     
	}
	return 0;
}
...
/**
  * @brief   Get the dMP-processed data (note that this function requires more stacks and a little too many local variables)
  * @param   pitch precision:0.1   Range:-90.0 <---> +90.0
  * @param   roll  precision:0.1   Range:-180.0<---> +180.0
  * @param   yaw   precision:0.1  Range:-180.0<---> +180.0
  * @retva   0:fail 1:success
  */
